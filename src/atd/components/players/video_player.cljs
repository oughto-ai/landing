(ns atd.components.players.video-player
...
(defnc video-player
  ...
  (let [video-ref (hooks/use-ref "video-ref")
        [audio-muted? set-audio-muted!] (hooks/use-state false)
    ...
    (d/div {:class "w-full h-full relative "}
           ...
           #_(d/div {:class "p-2 cursor-pointer absolute right-4 bottom-4 flex middle hover:text-white text-slate-300"
